...
if (! function_exists('statusColor'))
{
    ...
    {
        return Status::color((int) $status);
    }
}
...
if (! function_exists('ripeningProducts'))
{
    ...
    {
        $temp = [];
        foreach (\App\Models\Shop\Category::$sortRipeningProducts as $key => $value) {
            $temp[$key] = $value . \App\Models\Shop\Category::$ripeningDays[$key];
        }
        return $temp;
    }
}
