 <form method="POST" action="{{ route('register') }}" data-modules="register" class="text-start lg:py-10 py-8">
     @csrf
    <div class="grid grid-cols-1">
        <div class="mb-4" id="name-block">
            <label class="font-semibold" for="name">Your Name:</label>
            <input id="name" type="text" name="name" value="{{old('name')}}" class="@error('name') border-red-400 @enderror mt-3 w-full py-2 px-3 h-10 bg-transparent dark:bg-slate-900 dark:text-slate-200 rounded outline-none border border-gray-100 dark:border-gray-800 focus:ring-0" placeholder="Harry">
            <x-input-error :messages="$errors->get('name')" class="mt-2" />
        </div>

        <div class="mb-4" id="email-block">
            <label class="font-semibold" for="email">Email Address:</label>
            <input id="email" type="email" name="email" value="{{old('email')}}" class="@error('email') border-red-400 @enderror mt-3 w-full py-2 px-3 h-10 bg-transparent dark:bg-slate-900 dark:text-slate-200 rounded outline-none border border-gray-100 dark:border-gray-800 focus:ring-0" placeholder="name@example.com">
