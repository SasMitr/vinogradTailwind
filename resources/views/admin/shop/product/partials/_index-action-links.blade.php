<div class="flex flex-nowrap p-1 gap-1" id="nav">
    <div class="togglebutton out-line inline-block">
        <label for="toggle-{{$product->id}}" class="flex items-center cursor-pointer">
            <div class="relative">
                <input type="checkbox" id="toggle-{{$product->id}}" data-url="{{route('admin.product.toggle.status', ['product_id' => $product->id])}}" class="toggle-status sr-only" @checked($product->status)>
...
{{--    open-modal--}}
    <a href="{{route('admin.product.update.show', $product)}}" class="open-modal text-blue-700 hover:text-white border border-blue-700 hover:bg-blue-800 font-medium text-sm px-2 py-1 inline-flex items-center">
    ...
    <a href="{{route('shop.product', $product)}}"
        @class([
            'text-purple-700 hover:text-white border border-purple-700 hover:bg-purple-800 focus:ring-4 font-medium text-sm px-2 py-1 inline-flex items-center',
