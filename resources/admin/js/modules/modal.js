...
{
    constructor(width = '1200px') {
        this.modal = document.createElement('div');
        this.modal.className = 'fixed z-50 insert-0 bg-black/30 backdrop-blur-md overflow-y-auto h-full w-full top-0';
        this.modal.innerHTML = `
        <div class="modal-card relative max-h-full top-24 mx-auto" style="width: ${width}">
    ...
    insert(option){
        this.modal.querySelector('#modal-body').innerHTML = option.body;
        this.modal.querySelector('#modal-header').innerHTML = option.header;
    }

    getModule(modal) {
        return this.modal.querySelector('form').getAttribute('data-modules');
    }
}
